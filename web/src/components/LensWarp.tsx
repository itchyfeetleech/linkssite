...
export default function LensWarp({ k1 = 0.012, k2 = 0.002, center = { x: 0.5, y: 0.5 } }: Props) {
  ...
  vec3 beamPersist = beamRGB * beamLine * (0.015 * u_alive) * beamW;\n  vec3 prev=texture(u_prevPhi, vUv).rgb; vec3 decayVec = u_decayF * (1.0 - 0.35 * beamW); vec3 newPhi=max(baseLin + beamPersist, mix(vec3(0.0), prev*decayVec, u_alive)); vec3 bright=max(newPhi-vec3(0.6), vec3(0.0)); bright += beamAdd; oPhi=vec4(newPhi,1.0); oBright=vec4(bright,1.0);} `;
      const fs2 = `#version 300 es\nprecision highp float; out vec4 frag; in vec2 vUv;\nuniform sampler2D u_phi; uniform sampler2D u_bright; uniform sampler2D u_blue; uniform sampler2D u_normal; uniform sampler2D u_dirt;\nuniform vec2 u_res; uniform float u_alive; uniform float u_halo; uniform float u_time;\nvec3 bloom(vec2 uv){ vec2 px=1.0/u_res; vec3 s=vec3(0.0); for(int i=0;i<8;i++){ float a=6.2831853*float(i)/8.0; vec2 d=vec2(cos(a),sin(a)); s+=textureLod(u_bright, uv+d*px*1.5,1.0).rgb*0.20; s+=textureLod(u_bright, uv+d*px*2.5,2.0).rgb*0.12; s+=textureLod(u_bright, uv+d*px*4.0,3.0).rgb*0.08; } return s; }\nvoid main(){ vec3 phi=texture(u_phi,vUv).rgb; vec2 uv=vUv;\n  float dirt = texture(u_dirt, uv*1.0).r; vec3 bloomAll=bloom(uv); bloomAll *= mix(1.0, dirt*1.6, 0.6*u_alive);\n  vec3 haloText=clamp(bloomAll*(0.7*u_alive)*u_halo, vec3(0.0), 0.06*max(phi,vec3(0.0))*u_alive);\n  float luma = dot(phi, vec3(0.2126,0.7152,0.0722)); float dark = smoothstep(0.35, 0.0, luma); vec3 beamGlow=min(bloomAll*(mix(0.18,0.45,dark)*u_alive), vec3(mix(0.05,0.12,dark)));\n  vec3 colLin=phi+haloText+beamGlow;\n  vec3 nrm = texture(u_normal, uv*1.0).xyz * 2.0 - 1.0; nrm = normalize(nrm*vec3(0.6,0.6,1.0)); vec3 L = normalize(vec3(0.25,0.6,1.0)); vec3 V = vec3(0.0,0.0,1.0); vec3 H = normalize(L+V); float spec = pow(max(dot(nrm,H),0.0), 64.0); float specW = smoothstep(0.2, 0.7, luma); colLin += spec * (0.05*u_alive) * specW;\n  float tri=fract((uv.x*u_res.x)/3.0); vec3 mask=normalize(vec3(smoothstep(0.0,0.33,tri), smoothstep(0.33,0.66,tri), smoothstep(0.66,1.0,tri))+1e-3); colLin*=mix(vec3(1.0), mask*3.0, 0.03*u_alive);\n  float bn = texture(u_blue, uv*8.0 + vec2(0.017*u_time, -0.013*u_time)).r; colLin *= (1.0 + (bn-0.5)*(0.015*u_alive));\n  vec3 srgb=pow(max(colLin,vec3(0.0)), vec3(1.0/2.2)); float d=texture(u_blue, uv*6.0 + vec2(0.007*u_time, 0.011*u_time)).r; srgb += (d-0.5)*(0.7/255.0);\n  frag=vec4(clamp(srgb,0.0,1.0),1.0);} `;
      ...
      const u2 = { phi: gl2.getUniformLocation(prog2,"u_phi"), bright: gl2.getUniformLocation(prog2,"u_bright"), blue: gl2.getUniformLocation(prog2,"u_blue"), normal: gl2.getUniformLocation(prog2,"u_normal"), dirt: gl2.getUniformLocation(prog2,"u_dirt"), res: gl2.getUniformLocation(prog2,"u_res"), alive: gl2.getUniformLocation(prog2,"u_alive"), halo: gl2.getUniformLocation(prog2,"u_halo"), time: gl2.getUniformLocation(prog2,"u_time") } as const;
      ...
      const render = () => {
        ...
        if (u2.res) gl2.uniform2f(u2.res, cw, ch); if (u2.alive) gl2.uniform1f(u2.alive, effectiveAlive); if (u2.halo) gl2.uniform1f(u2.halo, haloRef.current); if (u2.time) gl2.uniform1f(u2.time, now*0.001);
        gl2.drawArrays(gl2.TRIANGLES, 0, 6);
